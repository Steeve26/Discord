...
export default function profile({icon, name, status, position, del}) {
  ...
  return (
    ...
    capitalize font-medium leading-5 gap-4 active:brightness-110 cursor-pointer group'>
      ...
      <div className="profile w-full h-full flex justify-between pr-4 text-[#949ba4] hover:text-[#dbdee1]">
        ...
        <button onClick={() => del(name)} className='hidden group-hover:block focus:block group-focus-visible:block hover:brightness-200'><BsX size={23}/></button>
      </div>
    </button>
  )
}
